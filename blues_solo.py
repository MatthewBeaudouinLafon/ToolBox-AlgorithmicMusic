...
licks = [   [[1,0.2],[1,0.2],[1,0.2],[-1,0.2]],  
            [[1,0.2],[-1,0.2],[-1,0.2],[-1,0.2]], 
            [[3,0.5],[-1,0.1],[-1,0.1],[-1,0.1]],  
            [[2,0.2],[-2,0.2],[2,0.2],[-2,0.2]], 
            [[1,0.3],[0,0.3],[2,0.1],[-2,0.3]], 
            #[[len(blues_scale)+5,0.3],[-3,0.2],[-len(blues_scale)+5,0.3],[2,0.4]] # For large jumps
        ]
lick_repeat = 2
volume = 1
for i in range(lick_repeat*len(licks)):
    lick = choice(licks)#licks[i/lick_repeat]
    crescendo = choice([-0.3, 0, 0.3])
    for note in lick:
        curr_note += note[0] 
        curr_note %= len(blues_scale) # If statements are for loosers
        volume += crescendo
        volume %= 2
